...
class Game:
    ...
    def createGame(self):
        ...
        self.time = 60
        for i in range(1):
            self.bgImgs.append(loadImage(path+'/superPusheenRes/bgImg.png'))
            #self.bgImgs.append(loadImage(path+'/superPusheenRes/layer_0'+str(i+1)+'.png'))
        
        #self.bgMusic=SoundFile(this, path+"/superPusheenRes/backgroundMusic.mp3")
        #self.bgMusic.amp(0.5)
        #self.bgMusic.play()

        f = open(path+"/superPusheenRes/stage"+str(self.stage)+".csv","r")
        for item in f:
            item = item.strip().split(",")
            if item[0] == "Pusheen":
                self.Pusheen = Pusheen(int(item[1]),int(item[2]),int(item[3]),int(item[4]),item[5],int(item[6]))
            elif item[0] == "Butter":
    ...
    def display(self):
        ...
        self.Pusheen.display()
        fill(255)
        textSize(32)
        text(str(self.score), 10, 25)
        text(str(self.time), 10, 50)
        
        if self.Pusheen.x >= self.stage_x_end:
            self.stage += 1
            self.createGame()
...
class Pusheen(Creature):
    def __init__(self,x,y,r,g,imgName,F):
        Creature.__init__(self,x,y,r,g,imgName,F)
        
        self.keyHandler={LEFT:False,RIGHT:False,UP:False,"P":False}
        #self.jumpSound = SoundFile (this, path+"/superPusheenRes/jump.mp3") 
        #self.killSound = SoundFile (this, path+"/superPusheenRes/kill.mp3")
...
def setup():
    size(game.w,game.h)
    game.createGame()
    img = loadImage(path+"/superPusheenRes/startImage.png")
    background(img)
...
def keyPressed():
    print (keyCode)
    game.Pusheen.keyHandler[keyCode]=True
    if keyCode == 80:
        game.paused = not game.paused

def keyReleased():
    game.Pusheen.keyHandler[keyCode]=False
